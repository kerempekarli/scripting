...
async function getMaxPage() {
  const maxPage = await DataModel.findOne({}, { pageNumber: 1 }, { sort: { pageNumber: -1 } }).lean() || { pageNumber: 0 } || 0
  const maxPageIndex = await DataModel.findOne(
    { pageNumber: maxPage.pageNumber },
    { pageIndex: 1 },
    { sort: { pageIndex: -1 }, limit: 1 }
  ).lean() || { pageIndex: 0 } || 0;

  console.log("Max Page:", maxPage.pageNumber);
  console.log("Max Page Index:", maxPageIndex.pageIndex);

  return {
    lastProcessedPageNumber: maxPage.pageNumber,
    lastProcessedPageIndex: maxPageIndex.pageIndex,
  };
}
